...
## 5. 交易时机
设计一个简单的交易策略。一般而言，平今仓的手续费要比非平金仓的手续费率要高得多，即$`α_1 < α_0`$，因此易得$`cost_1 < cost_0`$ 。交易逻辑为，当仓位为0，$`spread > up`$，且理论$`profit > cost_otherday`$，开仓交易。当仓位不为0，$`spread < up`$时，如果上个交易是否发生在当天，且满足$`profit > cost_today`$，则平今仓；如果上个交易未发生在当天，则平非金仓。
